...
export default function Ponencia({rawData}) {
    ...
    return (
        <Layout>
            <IntroSectionShort title={DatosPonencias[id].titulo !== "" ? DatosPonencias[id].titulo : DatosPonencias[id].empresa} />
            <Container>
                <PonenciaDiv>
                    {printImage(id)}
                    {printPonentes(id)}
...
const printImage = (id) => {
    if (DatosPonencias[id].logo !== "") {
        return (
            <AlignedDiv>
                <Image src={DatosPonencias[id].logo[0]} alt={DatosPonencias[id].titulo} width={DatosPonencias[id].logo[1]} height={DatosPonencias[id].logo[2]} />
            </AlignedDiv>
        )
    }
    else return "";
...
const printPonentes = (id) => {
    if (DatosPonencias[id].ponentes.length > 0) {
        ...
        return (
            DatosPonencias[id].ponentes.map((d) => {
                return (
                    <AlignedDiv key={d}>
...
const printTimeRoom = (id) => {
    ...
    return (
        <AlignedDiv>
            <Time>El {day} de {getTimePonencia(hor)} en la Sala {getRoom(ing)}</Time>
        </AlignedDiv>
    )
}
...
const getTimePonencia = (hor) => {
    switch (hor) {
        case '1': return '11:00 a 11:35';
        case '2': return '11:35 a 12:10';
        case '3': return '12:10 a 12:45';
        case '4': return '12:45 a 13:00';
        case '5': return '13:00 a 14:30';
        case '6': return '14:30 a 15:05';
        case '7': return '15:05 a 15:40';
        case '8': return '15:40 a 16:15';
        case '9': return '16:15 en adelante';
    }
}

const getRoom = (ing) => {
    switch (ing) {
        case '1': return 'Stephanie Shirley';
        case '2': return 'Frances Elizabeth Allen';
        case '3': return 'Pilar Careaga Basabe';
        case '4': return 'Margaret Heafield Hamilton';
        case '5': return 'Elisa Leonida Zamfirescu';
    }
}
